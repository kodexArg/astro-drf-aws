...
<!--
  ...
  `flex-1`, not `min-h-dvh`: `body` is the app-shell flex column, so this
  `<main>` fills exactly what the header stack leaves. A full-viewport
  min-height here would start below the header and end a header-height past
  the fold — the phantom scroll that pushed the standalone chat's composer
  off-screen. Long pages still stretch `body` past one viewport as before.
-->
...
<main class="flex w-full flex-1 flex-col p-12 pb-4 pt-16">
  {@render children?.()}
</main>
