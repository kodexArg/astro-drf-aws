...
**Harness**
- [[HARNESS]] — the skills this template requires, vendored into `.claude/skills/` + `skills/`; ruled by [[adr-02-harness]]. Stack/AWS work goes through these skills, vault `.md` through `obsidian-markdown`, fan-out through `kdx-orchestrator`.
...
- `docs/obsolete/` retired 2026-08-04: an ADR is now edited in place, its replaced policy recorded in that ADR's `REJECTED` section, never hollowed to a separate file ([[adr-00-adr-doctrine]] rule 8, REJECTED).
...
- `.claude/skills/` (git-tracked, harness-loaded) mirrored by `skills/`, and `hooks/` → `.claude/hooks/` — stage 2 homes. The required skills are **vendored real copies** (self-contained, no dependence on the machine-global skill harness), inventoried in [[HARNESS]] and ruled by [[adr-02-harness]].
