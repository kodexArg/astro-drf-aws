...
| `ASSISTANT_ENABLED` | backend | dev/prod/local | no | plain task env; `.env` local | Kill switch for the page-context `assistant` generating tier ([[CHATBOT]], [[adr-24-page-context-assistant]]): `false` short-circuits `POST /api/assistant/ask/` to `200 {outcome:"disabled",query_id}` with zero inference calls. Default `true` |
| `ASSISTANT_BEDROCK_MODEL_ID` | backend | dev/prod/local | no | plain task env; `.env` local | Bedrock model ID for the `assistant` generating-tier free-text answer ([[adr-24-page-context-assistant]]). Same cross-region inference-profile form as the router (`us.amazon.nova-micro-v1:0`). Reuses `BEDROCK_REGION` — no duplicate region variable. Call mechanism is `boto3` + `asgiref.sync_to_async` ([[adr-16-async-mandatory]] rule 4) |
| `ASSISTANT_USE_MOCK_INFERENCE` | backend | local | no | `.env` local (never a task env) | DEBUG-only opt-out from the deterministic assistant mock client: under `DEBUG` the assistant defaults to the mock; `false` selects real Bedrock locally. Never read outside `DEBUG` ([[adr-10-auth]] rule 6 precedent). Default `true` |
