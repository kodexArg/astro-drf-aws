...
- **The page crossfade.** `Base.astro` uses Astro's `ClientRouter` with a `fade({ duration: "150ms" })` transition on the page content area (`PageCanvas`); `NavDrawer` carries `transition:persist="shell-nav"` so the island does not remount — and therefore does not re-run its mount-time theme/migration effect (`readThemeCookie` + `migrateLegacyNavLock`; no viewport effect exists to re-run) — across a same-layout navigation.
