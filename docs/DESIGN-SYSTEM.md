...
- **Rail minimum width — `RAIL_MIN_WIDTH = "43.75rem"`.** Below this viewport width the rail is never presentable; `resolvePresentation` forces `"drawer"` even when the lock preference is `"locked"`, and the preference itself is untouched so the rail returns once the viewport widens again. `DESK_MIN_WIDTH = "64rem"` (Tailwind `lg`) is the tablet/desk band split, informational only — both bands present the rail once above the rail minimum.
...
- **Dual-mount at lock time.** When preference is `"locked"`, `NavDrawer` mounts both the rail and the drawer in the same paint and lets a `min-[43.75rem]:` CSS pair pick which is visible — this is what keeps a full page navigation from flashing unlocked while the client-side viewport check catches up.
