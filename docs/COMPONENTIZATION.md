...
| `nav-fsm` | `shell/` | Not a component: the DOM-free FSM `NavDrawer` and `Base.astro` share — preference (`nav_lock` cookie), presentation (`rail`\|`drawer`, derived from preference alone, never stored), active item. No viewport field: the rail-floor breakpoint is CSS alone ([[adr-28-nav-fsm-frosted-rail]]) | `NavDrawer` and `Base.astro` only |
