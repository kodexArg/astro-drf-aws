...
| RDS instance `alvs-prod-pg` | `alvs-prod-pg.cccpxuiv6n1v.us-east-1.rds.amazonaws.com` — PostgreSQL 17.9, `db.t4g.micro`, single-AZ, 20 GB, subnet group `alvs-prod-rds-subnets`, sg `sg-0c6f4c16f86f7a2b3`, master `postgres` (RDS-managed secret), not publicly reachable | shared | issue #14 (2026-08-05) — this project moved onto it; already hosting `alvs_financial_gateway` and `kcbd`. Never destroyed by this run's teardown |
| EC2 `monitor-prod-rds` | `i-09c2a789dc6c4e3f2` (`10.20.1.53`, sg `sg-0c40eb608221e107f`, SSM-managed) | shared | issue #14 (2026-08-05) — the SSM port-forward host for DB admin ([[BD]]); read-only use, never mutated |
...
- Database `astro_drf_aws` and login role `astro_drf_aws_user` on the shared `alvs-prod-pg` (issue #14, 2026-08-05 → dropped in E, instance untouched). `GRANT astro_drf_aws_user TO postgres` was applied on the master so it could create a database it does not own; that grant goes with the role.
...
| RDS subnet group `alvs-prod-astro-drf-aws-subnets` | isolated subnets `subnet-0f4865b1984f1ab22`, `subnet-0ad36358282f91a16` | destroyed 2026-08-05 | B2.3 (2026-07-12) — deleted with its instance (issue #14); the subnets themselves are shared and untouched |
| RDS instance `alvs-prod-astro-drf-aws-pg` | `alvs-prod-astro-drf-aws-pg.cccpxuiv6n1v.us-east-1.rds.amazonaws.com` — PostgreSQL 17.9, `db.t4g.micro`, single-AZ, 20 GB gp3, sg `sg-0c6f4c16f86f7a2b3`, db `app`, no deletion protection, no final snapshot | destroyed 2026-08-05 | B2.3 (2026-07-12) — retired by issue #14: [[adr-15-ephemeral-run]] rule 4's dedicated-instance policy was replaced (see that ADR's `REJECTED`), the data moved to `astro_drf_aws` on the shared `alvs-prod-pg`, and the instance was deleted with `--skip-final-snapshot --delete-automated-backups` after the live cutover verified |
