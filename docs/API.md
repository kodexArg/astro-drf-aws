...
Page-context **generating tier** ([[adr-24-page-context-assistant]], [[CHATBOT]]), an `async def` view on the existing ASGI server ([[adr-16-async-mandatory]]). RBAC gate ([[adr-10-auth]] rule 2): `IsAuthenticated` + `CanUseAssistant` — the requesting user must be a member of the Django group `admins` or `ai_operators`. Request body via `AskRequestSerializer`: `{ "utterance": "<user text>", "page": "</path/>" }` — `page` must be a member of the closed nav registry (`NAV_ITEMS`: `/`, `/chatui/`, `/showcase/components/`, `/profile/`, trailing slash required); any other value is `400`. The request carries a **page identity**, never page text or DOM content.

Server-side context is assembled from that page identity under the caller's Django Groups ([[adr-24-page-context-assistant]] rule 3). Inference (`get_assistant_inference_client`: real Bedrock in any non-DEBUG process, the deterministic mock only under `DEBUG`) runs through `asgiref.sync_to_async` ([[adr-16-async-mandatory]] rule 4). One `AssistantQuery` audit row is persisted per call that has an authenticated user, including the disabled, rate-blocked and unavailable paths.
...
- Unauthenticated → `401`/`403`; role-less authenticated session → `403` (lobby, [[adr-20-authorization-lobby]]).
